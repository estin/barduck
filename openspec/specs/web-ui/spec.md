...
A layout cell that groups several source ids into one pane SHALL render as a single card titled with the group's configured title, containing one row per grouped value. Each row SHALL show that value's configured label and its latest value and unit, and SHALL be colored independently — the same health-first coloring rule single-source panels use (spec: web-ui — health visible at a glance), applied per row — so one row in a group pane can be red while another is green. A row's coloring is its value's text color only — the row's label is not colored, and unlike a single-source panel, a group row carries no border or background color of its own. A row that is currently failing or stale SHALL also show a plain, uncolored status label next to it, alongside its color, the same as a single-source panel's. A row for a healthy source with no threshold bands gets no accent color at all, same as a single-source panel.

The card's own border — not its background — SHALL be colored by the worst color among its rows, ranked red > yellow > green, using the same health-first rule as each row's own color: a row failing or stale contributes red or yellow regardless of its band; a healthy row contributes its band color, or green if unbanded. The card's background stays neutral regardless of its rows' colors.

Each row's label SHALL be a link to `/logs/<source>` for that row's source, using the same log view a single-source panel's footer link opens. A row SHALL show its "updated X ago" age text only when that source is lagging (its health status is `stale`); a row that is not stale shows no age text at all, keeping fresh rows compact.

Each row whose source declares threshold bands SHALL render its own history-bar preview below the row, using the same segments/coloring and `show_history` opt-out as a single-source panel's history bar (spec: web-ui — Panel retrospective history bar). A row whose source declares no threshold bands renders no history bar, same as a single-source panel.
...
- **THEN** the web UI renders one card titled with the group's title, containing a "days left" row and a "balance" row, each showing that source's latest value and unit

#### Scenario: Rows colored independently
- **WHEN** a group pane's `days left` value is healthy and inside its red threshold band, and its `balance` value is healthy and inside its green threshold band
...
- **WHEN** a group pane's rows currently render green, yellow, and red
...
- **WHEN** a group pane has one threshold-banded row currently green (healthy) and one unbanded row whose source is `failing`
...
- **WHEN** a group pane's "days left" row is healthy and inside its red threshold band
...
- **WHEN** a group pane's "balance" row is for a source with no threshold bands and that source is currently `failing`
...
- **WHEN** a group pane's "balance" row is for source `vds-base1-balance`
- **THEN** that row's "balance" label links to `/logs/vds-base1-balance`

#### Scenario: Age hidden for a row that isn't lagging
- **WHEN** a group pane's "balance" row was just collected (not `stale`)
- **THEN** that row shows no "updated X ago" text

#### Scenario: Age shown for a lagging row
- **WHEN** a group pane's "days left" row's source has gone `stale`
- **THEN** that row shows its "updated X ago" age

#### Scenario: Row shows its own history bar
- **WHEN** a group pane's "days left" row is healthy, for a threshold-banded source with recent readings
...
- **WHEN** a group pane's "days left" row is healthy, for a threshold-banded source that declares `show_history = false`
...
- **WHEN** a group pane's "balance" row is for a source with no threshold bands and that source is currently healthy
...
- **WHEN** a group pane's "days left" row is for a threshold-banded source whose last known value fell in its red band, but the source is now `stale`
