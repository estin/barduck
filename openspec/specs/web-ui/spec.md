...
- **THEN** the rendered pane is a table with one row per child, in declared order, each colored by that child's own health and thresholds
...
- **THEN** the pane renders the children table in place of a single scalar value
